...
let appView='', itemId='', appReplyBusy=false, marketplaceRefreshBusy=false;
...
function settingsRoot(){
  const c=ctx();if(!c?.extensionSettings)return{profiles:{},identities:{},socialProfiles:{}};
...
function normalizeContact(x={}){
  x.id||=`contact:${id()}`;x.name=norm(x.name||'Unknown');x.avatar||='';x.characterId=Number.isInteger(Number(x.characterId))?Number(x.characterId):null;x.personaDescription=String(x.personaDescription||'');x.source||='manual';x.identityId=String(x.identityId||'');x.phoneNumber=cleanPhoneNumber(x.phoneNumber);x.saved=x.saved!==false;x.socialSeedVersion=Math.max(0,Number(x.socialSeedVersion||0));x.favorite=x.favorite===true;x.blocked=x.blocked===true;x.blockedByContact=x.blockedByContact===true;x.ignoringOwner=x.ignoringOwner===true;x.boundaryLevel=Math.max(0,Number(x.boundaryLevel||0)||0);x.muted=x.muted===true;x.locationSharing=['precise','approximate','off'].includes(x.locationSharing)?x.locationSharing:'precise';x.nickname||='';return x
}
...
async function refreshPhone(){
  ...
  try{
    ...
    if(p.apps.messages)allowedLines.push('- messages: message.receive');if(p.apps.phone)allowedLines.push('- phone: phone.call');if(p.apps.instagram)allowedLines.push('- instagram: instagram.follow, instagram.unfollow, instagram.post, instagram.story, instagram.dm, instagram.story.view, instagram.post.like, instagram.post.comment');if(p.apps.snapchat)allowedLines.push('- snapchat: snapchat.add, snapchat.accept, snapchat.decline, snapchat.snap, snapchat.story, snapchat.npc.request');if(p.apps.facebook)allowedLines.push('- facebook: facebook.friend.request, facebook.friend.accept, facebook.friend.decline, facebook.post, facebook.post.like, facebook.post.comment, facebook.messenger');if(p.apps.mail)allowedLines.push('- mail: mail.receive');
    if(!enabledApps.length){globalThis.toastr?.warning?.('No refresh-capable apps are installed.');return}if(!candidates.length&&!p.apps.mail&&!p.apps.snapchat){globalThis.toastr?.warning?.('No relevant phone identities yet.');return}
...
function addMarketplaceListing(t,data,{ownerListing=false}={}){
  ...
  const title=String(data.title||'').trim(),description=String(data.description||'').trim(),price=String(data.price||'').trim(),area=String(data.area||'').trim(),visual=String(data.visual||data.mediaDescription||'').trim();if(!seller||!title||!price)return null;
  const row={id:id(),sellerIdentityId:seller.id,sellerName:seller.name,sellerType:seller.kind==='provisional'?'provisional':'existing',title,description,price,area,visual,type:data.type==='video'?'video':'photo',mediaKey:String(data.mediaKey||''),mediaWidth:Math.max(0,Number(data.mediaWidth||0)),mediaHeight:Math.max(0,Number(data.mediaHeight||0)),timeMs:now().getTime(),status:'available',linkedManually:false,ownerListing:ownerListing||seller.id===owner?.id,inquiries:[]};row.signature=marketplaceListingSignature(row);if(t.facebook.marketplace.listings.some(x=>x.signature===row.signature||marketplaceListingSignature(x)===row.signature))return null;return row;
}
function applyMarketplaceRefresh(result){
  const t=timeline(),market=t.facebook.marketplace,keys=new Set(market.refresh.eventKeys||[]),newRows=[];let count=0;
  for(const data of result.listings){const row=addMarketplaceListing(t,data);if(!row||keys.has(`listing:${row.signature}`))continue;keys.add(`listing:${row.signature}`);newRows.push(row);count++}
  if(newRows.length)market.listings.unshift(...newRows);
  const owner=currentIdentity(),candidates=marketplaceCandidateContext(),byId=new Map(candidates.map(x=>[x.identityId,identityById(x.identityId)])),byName=new Map(candidates.map(x=>[lc(x.name),identityById(x.identityId)]));
  for(const data of result.buyerMessages){const listing=market.listings.find(x=>x.ownerListing&&(x.id===data.listingId||!data.listingId&&lc(x.title)===lc(data.listingTitle)))||market.listings.find(x=>x.ownerListing&&x.status==='available');if(!listing)continue;const buyerType=data.buyerType==='existing'?'existing':'provisional',buyer=buyerType==='existing'?(byId.get(String(data.buyerIdentityId||''))||byName.get(lc(data.buyer))):createProvisionalIdentity(data.buyer||data.firstName||'Buyer',{source:'marketplace-buyer',interestedListingId:listing.id});const body=String(data.text||'').trim();if(!buyer||buyer.id===owner?.id||!body)continue;const key=`buyer:${listing.id}:${buyer.id}:${hashString(body)}`;if(keys.has(key))continue;const delivered=mirrorAppMessage({appName:'facebook',fromName:buyer.name,toName:owner.name,fromIdentityId:buyer.id,toIdentityId:owner.id,type:'text',text:body,timeMs:now().getTime(),unread:true});if(!delivered?.delivered)continue;keys.add(key);listing.inquiries=Array.isArray(listing.inquiries)?listing.inquiries:[];listing.inquiries.push({identityId:buyer.id,name:buyer.name,timeMs:now().getTime(),message:body});count++}
  market.refresh.lastAt=Date.now();market.refresh.eventKeys=[...keys].slice(-120);market.refresh.summary=String(result.summary||'').trim();persist(t,false);return count;
}
async function refreshMarketplace(){
  ...
  try{
    const t=timeline(),owner=currentIdentity(),w=world(),candidates=marketplaceCandidateContext(),existing=t.facebook.marketplace.listings.slice(0,40).map(x=>({id:x.id,seller:x.sellerName,title:x.title,price:x.price,area:x.area,status:x.status,ownerListing:x.ownerListing})),ownerListings=t.facebook.marketplace.listings.filter(x=>x.ownerListing&&x.status==='available').map(x=>({id:x.id,title:x.title,description:x.description,price:x.price,area:x.area}));
...
Create 5 to 8 NEW plausible listings that do not duplicate EXISTING LISTINGS. Old listings will remain; you are appending new inventory.
...
Prices should be plausible and normally use euros. Areas should fit Greyhaven without claiming unsupported precise live locations.
If PHONE OWNER LISTINGS exist, you may also create 0 to 2 buyerMessages from a plausible interested existing identity or first-name-only provisional buyer. A buyer may ask a question, negotiate, make an offer, arrange pickup or say they want to buy. Do not invent a completed sale.
Required JSON:
...
function applyRefresh(r){
  ...
  for(const x of events){
    ...
    if(!appName||!installed[appName]||type.startsWith('marketplace.'))continue;if(!['mail.receive','snapchat.npc.request'].includes(type)&&!identity)continue;
...
function renderSnapchat(){
  ...
  if(view==='camera')body=`<section class="ghp-snap-camera"><i class="fa-solid fa-camera"></i><h2>Send a Snap</h2><p>Describe a photo or video for the recipient AI. A local upload can replace the placeholder visually.</p><button data-compose-snap><i class="fa-regular fa-circle"></i></button></section>`;
...
function renderFacebook(){
  const t=timeline(),view=appView||'feed',store=t.facebook,own=currentIdentity(),counts=socialCounts(t,own);
  ...
  else if(view==='marketplace')body=`<div class="ghp-market-tools"><button data-marketplace-refresh ${marketplaceRefreshBusy?'disabled':''}><i class="fa-solid fa-rotate"></i><span><b>${marketplaceRefreshBusy?'Refreshing…':'Refresh listings'}</b><small>Dedicated Marketplace AI refresh</small></span></button><button data-new-market-listing><i class="fa-solid fa-tag"></i><span><b>Sell an item</b><small>Create your own listing</small></span></button></div><div class="ghp-market-grid">${store.marketplace.listings.map(row=>`<button data-market-listing="${esc(row.id)}">${socialMediaCard(row,'Listing')}<b>${esc(row.price)}</b><span>${esc(row.title)}</span><small>${esc(row.area||row.sellerName)}${row.ownerListing?' · Your listing':''}</small></button>`).join('')||empty('fa-solid fa-store','No listings yet','Tap Refresh listings to append a Marketplace batch without spending the main Phone Refresh request.')}</div>`;
...
function expose(){globalThis.GreyhavenPhone={version:GHP_VERSION,open:openPhone,close:closePhone,refresh:refreshPhone,refreshMarketplace,getProfile:()=>clone(profile()),getTimeline:()=>clone(timeline(false)),getContacts:()=>{const t=timeline(false);return clone(t?t.contactOrder.map(k=>t.contacts[k]).filter(Boolean):[])},getActivePersona:()=>clone(persona()),getContinuitySnapshot:()=>clone(continuitySnapshot()),getPromptSummary:()=>phonePromptSummary(),getIdentityPromptSummary:()=>identityPromptSummary(),listIdentities:()=>clone(Object.values(ensureAllIdentities())),getIdentityById:key=>clone(identityById(key)),getIdentityByName:name=>clone(identityForName(name,{create:true})),getIdentityByNumber:number=>clone(identityByNumber(number)),getCurrentIdentity:()=>clone(currentIdentity()),getSocialProfile:identity=>clone(socialProfileFor(resolveIdentityRef(identity,false)||currentIdentity())),getSocialCounts:()=>clone(socialCounts(timeline(),currentIdentity())),setIdentityPhoneNumber:(identityId,number)=>{const updated=updateIdentityNumber(identityId,number);propagateIdentityNumber(updated);return clone(updated)},saveContactForOwner:(owner,target,options)=>saveContactForOwner(owner,target,options),seedContacts:()=>clone(seedContacts(true)),removeContact,actionBus:{dispatch:(action,options)=>globalThis.GreyhavenLife?.dispatchWorldAction?.(action,options)||false},apps:{sendMessage:mirrorAppMessage,followInstagram:setInstagramFollowing,requestSnapchat:setSnapchatFriendRequest,requestFacebook:setFacebookFriendRequest,engagePost:updateSharedSocialEngagement,publishItem:publishAppItem,createMarketplaceListing:data=>{const t=timeline(),row=addMarketplaceListing(t,data,{ownerListing:true});if(!row)return null;t.facebook.marketplace.listings.unshift(row);persist(t,false);return clone(row)}}}}
