...
let appView='', itemId='', appReplyBusy=false, marketplaceRefreshBusy=false, onlyFansRefreshBusy=false, darkWebRefreshBusy=false;
let composeRequest={threadId:'',kind:'',appName:''};
...
function identityCardSignals(identity){
  const found=findCharacter(identity?.name),card=cardData({name:identity?.name||'',characterId:found?.index??null}),source=[identity?.name,card.description,card.personality,card.scenario,identity?.metadata?.bio,identity?.metadata?.source].filter(Boolean).join('\n'),text=lc(source);
  return{card,text,signature:hashString(source),provisional:identity?.kind==='provisional',onlyFans:/\bonly\s*fans\b|\bonlyfans\b/.test(text),influencer:/\binfluencer\b|\bcontent creator\b|\bsocial media (?:star|personality)\b|\binstagram model\b|\bstreamer\b/.test(text),explicitFame:/\b(?:very|highly|widely|well)[ -]?known\b|\bcelebrity\b|\bfamous\b|\bviral\b|\btop creator\b|\bhuge following\b|\bmassive following\b|\bsuccessful influencer\b/.test(text),lowProfile:/\bnot (?:(?:very|too|that) )?(?:famous|well[- ]known|popular)\b|\bsmall (?:following|audience|creator|account)\b|\bmodest (?:following|audience)\b|\blow[- ]key\b|\bprivate person\b|\bnew creator\b|\bjust started\b|\bstruggling creator\b|\bniche audience\b/.test(text),publicFacing:/\bmodel\b|\bmusician\b|\bartist\b|\bdancer\b|\bcreator\b|\bpublic figure\b|\bpopular\b/.test(text),feminine:/\b(?:she|her|hers|woman|women|girl|female|girlfriend|wife|mother|nurse)\b/.test(text),attractive:/\b(?:attractive|beautiful|gorgeous|pretty|stunning|hot|glamorous|sexy)\b/.test(text)};
}
function inferredSocialProfile(identity){
  const s=identityCardSignals(identity),{provisional,onlyFans,influencer,explicitFame,lowProfile,publicFacing,feminine,attractive}=s;
  ...
  if(provisional){
    ...
    tier=onlyFans||influencer?'small-creator':'low-profile';followers=seededSocialNumber(identity,'ig-followers',onlyFans||influencer?180:90,onlyFans||influencer?850:520);following=seededSocialNumber(identity,'ig-following',80,430);snapFriends=seededSocialNumber(identity,'snap-friends',45,260);facebookFriends=seededSocialNumber(identity,'fb-friends',90,620);
  }else if(explicitFame){
    tier='influencer';followers=seededSocialNumber(identity,'ig-followers',8500,32000);following=seededSocialNumber(identity,'ig-following',180,980);snapFriends=seededSocialNumber(identity,'snap-friends',320,1800);facebookFriends=seededSocialNumber(identity,'fb-friends',420,2200);
  }else if(influencer||onlyFans){
    tier=onlyFans?'creator':'influencer';followers=seededSocialNumber(identity,'ig-followers',onlyFans?700:2200,onlyFans?4800:9500);following=seededSocialNumber(identity,'ig-following',150,850);snapFriends=seededSocialNumber(identity,'snap-friends',180,950);facebookFriends=seededSocialNumber(identity,'fb-friends',260,1500);
  }else if(publicFacing){
    tier='public';followers=seededSocialNumber(identity,'ig-followers',900,5200);following=seededSocialNumber(identity,'ig-following',180,900);snapFriends=seededSocialNumber(identity,'snap-friends',150,720);facebookFriends=seededSocialNumber(identity,'fb-friends',260,1350);
  }else if(feminine){
  ...
  return{version:2,identityId:identity?.id||'',cardSignature:s.signature,tier,generatedAt:Date.now(),instagram:{followersBase:followers,followingBase:following},snapchat:{friendsBase:snapFriends},facebook:{friendsBase:facebookFriends}};
}
function normalizeSocialProfile(raw={},identity=null){
  ...
  const rebase=Number(x.version||0)<2||String(x.cardSignature||'')!==fallback.cardSignature;if(rebase)return fallback;
  x.version=2;x.identityId=identity?.id||String(x.identityId||'');x.cardSignature=fallback.cardSignature;x.tier=String(x.tier||fallback.tier);x.generatedAt=Math.max(0,Number(x.generatedAt||fallback.generatedAt));
...
function socialProfileFor(identityRef){
  ...
  const root=settingsRoot(),raw=root.socialProfiles[identity.id],existed=!!raw,value=normalizeSocialProfile(raw,identity),changed=!existed||Number(raw?.version||0)<2||String(raw?.cardSignature||'')!==value.cardSignature;root.socialProfiles[identity.id]=value;
  if(changed)ctx()?.saveSettingsDebounced?.();return value;
}
...
function seedOnlyFansPosts(identity,joinedAt){
  const pool=[['late-night members-only drop 💙','A flirtatious bedroom mirror selfie in warm low light','photo'],['a little behind-the-scenes moment','A candid dressing-room photo with a playful expression','photo'],['you asked, so I finally posted it 😏','A teasing close-up video filmed from bed','video'],['soft light, no rush, just me','An intimate boudoir portrait in soft window light','photo'],['couldn’t sleep so… 👀','A short after-midnight video with a mischievous smile','video'],['today’s set was way too fun','A glamorous studio outtake from a private photoshoot','photo'],['morning, trouble ☕','A relaxed morning selfie wrapped in a bedsheet','photo'],['this one stayed off the public feed','A daring private mirror photo with artful framing','photo'],['tell me which look wins','A changing-room video showing two bold outfits','video'],['weekend mood 😈','A playful poolside photo intended for subscribers','photo'],['no filter tonight','A natural close portrait in low bedroom lighting','photo'],['a preview before the full set','A sensual teaser clip from an upcoming private set','video']];
  const start=seededSocialNumber(identity,'of-seed-start',0,pool.length-1);return Array.from({length:4},(_,index)=>{const [caption,visual,type]=pool[(start+index*3)%pool.length];return normalizeOnlyFansPost({id:`onlyfans-seed:${hashString(identity.id)}:v2:${index}`,identityId:identity.id,author:identity.name,type,caption,visual,timeMs:Math.max(joinedAt,Date.now()-(index+2)*86400000*11),likes:seededSocialNumber(identity,`of-like-v2-${index}`,12,720),comments:seededSocialNumber(identity,`of-comment-v2-${index}`,1,55),seeded:true},identity)});
}
function inferredOnlyFansAccount(identity){
  const s=identityCardSignals(identity),social=socialProfileFor(identity);let economicTier='emerging';if(s.lowProfile)economicTier='small';else if(s.explicitFame)economicTier='star';else if(social.tier==='influencer')economicTier='established';else if(s.onlyFans||social.tier==='creator'||social.tier==='public')economicTier='creator';
  const ranges={small:{subs:[8,70],monthly:[35,130],total:[250,1200],posts:[12,55]},emerging:{subs:[18,120],monthly:[55,190],total:[350,1900],posts:[16,70]},creator:{subs:[45,260],monthly:[100,380],total:[850,4200],posts:[24,110]},established:{subs:[120,520],monthly:[320,720],total:[2800,7600],posts:[45,165]},star:{subs:[280,900],monthly:[650,1000],total:[6000,10000],posts:[70,220]}}[economicTier],subscribers=seededSocialNumber(identity,'of-subscribers-v2',...ranges.subs),price=Number((seededSocialNumber(identity,'of-price-v2',6,19)+.99).toFixed(2)),historicalPosts=seededSocialNumber(identity,'of-posts-v2',...ranges.posts),months=seededSocialNumber(identity,'of-months-v2',6,30),joinedAt=Date.now()-months*30*86400000,monthly=seededSocialNumber(identity,'of-monthly-v2',...ranges.monthly),total=Math.max(monthly,seededSocialNumber(identity,'of-total-v2',...ranges.total)),posts=seedOnlyFansPosts(identity,joinedAt);
  return{version:2,identityId:identity.id,cardSignature:s.signature,economicTier,enabled:true,subscriptionPrice:price,subscribersBase:subscribers,historicalPostCount:historicalPosts,monthlyEarningsBase:Math.min(1000,monthly),totalEarningsBase:Math.min(10000,total),joinedAt,posts,activity:[{id:`of-activity:${hashString(identity.id)}:1`,text:'Your latest post is performing better than usual.',timeMs:Date.now()-2*86400000},{id:`of-activity:${hashString(identity.id)}:2`,text:`${seededSocialNumber(identity,'of-new-subs-v2',1,economicTier==='star'?24:9)} new subscribers this week.`,timeMs:Date.now()-5*86400000},{id:`of-activity:${hashString(identity.id)}:3`,text:'A returning subscriber sent a private message.',timeMs:Date.now()-9*86400000}]};
}
function normalizeOnlyFansAccount(raw={},identity){
  const fallback=inferredOnlyFansAccount(identity),x=raw&&typeof raw==='object'?raw:{},rebase=Number(x.version||0)<2||String(x.cardSignature||'')!==fallback.cardSignature,manualPosts=Array.isArray(x.posts)?x.posts.map(row=>normalizeOnlyFansPost(row,identity)).filter(row=>!row.seeded):[];
  if(rebase){fallback.enabled=x.enabled!==false;fallback.subscriptionPrice=Math.max(1,Math.min(500,Number(x.subscriptionPrice??fallback.subscriptionPrice)));fallback.posts=[...manualPosts,...fallback.posts].sort((a,b)=>b.timeMs-a.timeMs).slice(0,80);fallback.activity=Array.isArray(x.activity)&&x.activity.length?x.activity.filter(Boolean).slice(0,80):fallback.activity;return fallback}
  x.version=2;x.identityId=identity.id;x.cardSignature=fallback.cardSignature;x.economicTier=String(x.economicTier||fallback.economicTier);x.enabled=x.enabled!==false;x.subscriptionPrice=Math.max(1,Math.min(500,Number(x.subscriptionPrice??fallback.subscriptionPrice)));x.subscribersBase=Math.max(1,Math.round(Number(x.subscribersBase??fallback.subscribersBase)));x.historicalPostCount=Math.max(1,Math.round(Number(x.historicalPostCount??fallback.historicalPostCount)));x.monthlyEarningsBase=Math.max(0,Math.min(1000,Math.round(Number(x.monthlyEarningsBase??fallback.monthlyEarningsBase))));x.totalEarningsBase=Math.max(x.monthlyEarningsBase,Math.min(10000,Math.round(Number(x.totalEarningsBase??fallback.totalEarningsBase))));x.joinedAt=Math.max(0,Number(x.joinedAt||fallback.joinedAt));x.posts=Array.isArray(x.posts)?x.posts.map(row=>normalizeOnlyFansPost(row,identity)).slice(0,80):fallback.posts;x.activity=Array.isArray(x.activity)?x.activity.filter(Boolean).slice(0,80):fallback.activity;return x;
}
function onlyFansAccount(identityRef,create=false){
  const identity=typeof identityRef==='string'?(identityById(identityRef)||identityForName(identityRef,{create:false})):identityRef;if(!identity?.id)return null;const root=settingsRoot(),roles=globalAppRoles(),assigned=roles.onlyfans.creatorIds.includes(identity.id),raw=root.onlyFansAccounts[identity.id];if(!assigned&&!create)return null;const account=normalizeOnlyFansAccount(raw||{},identity),changed=!raw||Number(raw?.version||0)<2||String(raw?.cardSignature||'')!==account.cardSignature;account.enabled=assigned;root.onlyFansAccounts[identity.id]=account;if(changed)ctx()?.saveSettingsDebounced?.();return account;
}
...
function normalizeAppThreads(store={}){
  ...
  for(const [key,value] of Object.entries(store.threads)){const th=value&&typeof value==='object'?value:{};th.id=key;th.identityId=String(th.identityId||'');th.peerName=norm(th.peerName||'Unknown');th.peerAvatar=String(th.peerAvatar||'');th.messages=Array.isArray(th.messages)?th.messages.map(normalizeAppMessage):[];th.createdAt=Math.max(0,Number(th.createdAt||Date.now()));th.contextListingId=String(th.contextListingId||'');th.videoActive=th.videoActive===true;th.videoStartedAt=Math.max(0,Number(th.videoStartedAt||0));th.videoSessionId=String(th.videoSessionId||'');th.videoTranscript=Array.isArray(th.videoTranscript)?th.videoTranscript.map(normalizeAppMessage):[];th.videoCallHistory=Array.isArray(th.videoCallHistory)?th.videoCallHistory.filter(Boolean).slice(-12).map(call=>({id:String(call?.id||id()),startedAt:Math.max(0,Number(call?.startedAt||0)),endedAt:Math.max(0,Number(call?.endedAt||0)),transcript:Array.isArray(call?.transcript)?call.transcript.map(normalizeAppMessage):[]})):[];store.threads[key]=th}
...
function mirrorAppMessage({appName,fromName,toName,fromIdentityId='',toIdentityId='',type='text',text='',mediaDescription='',mediaKey='',mediaWidth=0,mediaHeight=0,requestMedia='',timeMs=now().getTime(),mirrorId='',unread=true}){
  ...
  const shared=mirrorId||`${appName}:${id()}`,message=normalizeAppMessage({id:id(),mirrorId:shared,sender:fromIdentity.name,senderId:fromIdentity.id,type,text,mediaDescription,mediaKey,mediaWidth,mediaHeight,requestMedia:['photo','video'].includes(requestMedia)?requestMedia:'',timeMs,realMs:Date.now(),read:true,opened:appName!=='snapchat'||type==='text'});
...
function openAppConversation(appName,identityId,context={}){
  const identity=identityById(identityId);if(!identity)return;const t=timeline(),th=ensureAppThreadIn(t,appName,identity);if(context.listingId)th.contextListingId=String(context.listingId);persist(t,false);app=appName;appView='thread';itemId=th.id;render();
}
...
function darkWebProviderContext(identity){
  if(!identity?.id)return null;const roles=globalAppRoles().darkweb,sections=[['drugs','dealers'],['escorts','escorts'],['crime','crime'],['intel','intel']],assigned=[];for(const [section,role] of sections){const config=roles?.[role]?.[identity.id];if(config)assigned.push({section,role,price:String(config.price||''),duration:String(config.duration||''),description:String(config.description||'')})}const listings=sharedDarkWeb().listings.filter(x=>x.providerIdentityId===identity.id).map(x=>({id:x.id,section:x.section,title:x.title,description:x.description,price:x.price,duration:x.duration,area:x.area,status:x.status}));return assigned.length||listings.length?{assignedRoles:assigned,activeListings:listings}:null;
}
async function generateAppReply(appName,threadKey){
  ...
  const owner=persona(),ownerIdentity=currentIdentity(),videoMode=appName==='onlyfans'&&th.videoActive,conversation=videoMode?th.videoTranscript:th.messages,last=conversation.at(-1);if(!last||lc(last.sender)!==lc(owner.name))return;
  appReplyBusy=true;islandText=`${peer.name}…`;islandIcon=APPS[appName]?.icon||'fa-solid fa-ellipsis';render();
  try{
    const found=findCharacter(peer.name),w=world(),life=w.people.find(p=>lc(p.name)===lc(peer.name))||null,reln=t.relationships?.[peer.id]||null,listing=appName==='facebook'?sharedMarketplace().listings.find(x=>x.sellerIdentityId===peer.id)||null:null,darkState=appName==='darkweb'?sharedDarkWeb():null,darkListing=appName==='darkweb'?(darkState.listings.find(x=>x.id===th.contextListingId)||darkState.listings.find(x=>x.providerIdentityId===peer.id)||null):null,darkProvider=appName==='darkweb'?darkWebProviderContext(peer):null,creatorAccount=appName==='onlyfans'?onlyFansAccount(peer,false):null,requested=['photo','video'].includes(last.requestMedia)?last.requestMedia:'';
...
${requested&&appName!=='onlyfans'?`The latest message explicitly requests a ${requested}. Decide naturally from relationship, trust, mood and boundaries. If ${peer.name} sends it now, include an actual ${requested.toUpperCase()}: line; never merely claim it was sent.`:''}
...
${appName==='onlyfans'?`Every creator and subscriber in this app is confirmed age 18 or older. Keep the response consensual and consistent with the creator’s actual boundaries; subscription never erases consent. This is a paid creator context, so flirting, dirty talk, intimate performance, and requested private media are normal possibilities when they fit ${peer.name}'s creator persona and the conversation. Do not default to a generic "I'm not like that" response or moral lecture merely because the request is sexual. A creator may still tease, negotiate, redirect, or refuse something outside their own boundaries.${requested?` The latest message explicitly requests a ${requested}. If ${peer.name} sends it now, include an actual ${requested.toUpperCase()}: protocol line with a concrete visual description; never merely claim it was sent.`:''}`:''}
${appName==='darkweb'?`Keep everything inside fictional Greyhaven. Do not provide real vendor details, real-world sourcing, manufacturing, evasion, or operational instructions. DARK WEB PROVIDER CONTEXT below is authoritative. If ${peer.name} is explicitly assigned as an escort/dealer/provider, they know that role, their exact price, duration, description and boundaries even when no generated listing exists. Do not deny the assigned role or say "I'm not like that" simply because this chat was opened directly. The character can negotiate, refuse requests outside the listed boundaries, or arrange a fictional RP meeting.`:''}
...
${JSON.stringify({identity:{id:peer.id,name:peer.name,kind:peer.kind,phoneNumber:peer.phoneNumber||'',metadata:peer.metadata},character:found?cardData({name:peer.name,characterId:found.index}):{},life,sharedRelationship:reln,marketplaceListing:listing,darkWebListing:darkListing,darkWebProvider:darkProvider,onlyFansCreator:creatorAccount?{subscriptionPrice:creatorAccount.subscriptionPrice,subscribers:creatorAccount.subscribersBase,posts:creatorAccount.historicalPostCount,economicTier:creatorAccount.economicTier}:null,videoMode})}
...
${videoMode?'THIS PRIVATE VIDEO-CALL TRANSCRIPT (kept out of DMs)':'THIS APP THREAD (independent from iMessage and other apps)'}:
${JSON.stringify(conversation.slice(-24).map(messageContext))}

Choose ${peer.name}'s one bounded response now. ACTION: IGNORE is valid.`;
    let raw=await generate({prompt,systemPrompt,responseLength:680}),packet=parseDirectPacket(raw,requested);
    if(requested&&!hasExplicitMediaPacket(packet.items,requested)&&!packet.action&&!mediaRefusalText(raw)&&mediaSendClaim(raw,requested)){
      raw=await generate({prompt:`${prompt}\n\nDELIVERY CORRECTION: The previous draft claimed to send a ${requested} without attaching it. If ${peer.name} is still willing, return optional TEXT: plus a concrete ${requested.toUpperCase()}: line now. Otherwise return a clear in-character TEXT refusal or boundary. Do not claim delivery without the media protocol.`,systemPrompt,responseLength:520});packet=parseDirectPacket(raw,requested);
    }
    for(const row of packet.items.slice(0,3)){if(videoMode){const spoken=String(row.text||'').trim(),shown=String(row.mediaDescription||'').trim(),text=[spoken,shown?`*shows ${shown}*`:''].filter(Boolean).join(' ');if(text)appendOnlyFansVideoTurn(peer,ownerIdentity,text)}else mirrorAppMessage({appName,fromName:peer.name,toName:owner.name,fromIdentityId:peer.id,toIdentityId:ownerIdentity.id,type:row.type,text:row.text||'',mediaDescription:row.mediaDescription||'',timeMs:now().getTime(),unread:false})}
...
function sendAppMessage(appName,threadKey,{type='text',text='',mediaDescription='',mediaKey='',mediaWidth=0,mediaHeight=0,requestMedia=''}={}){
  ...
  const result=mirrorAppMessage({appName,fromName:owner.name,toName:peer.name,fromIdentityId:owner.id,toIdentityId:peer.id,type,text,mediaDescription,mediaKey,mediaWidth,mediaHeight,requestMedia,timeMs:now().getTime(),unread:true});render();if(result?.delivered)setTimeout(()=>generateAppReply(appName,threadKey),180);return !!result?.delivered;
}
...
function sharedOnlyFans(){
  const root=metadataRoot();if(!root)return{version:2,subscriptions:{},refresh:{lastAt:null,eventKeys:[],summary:''}};const state=root.onlyFans&&typeof root.onlyFans==='object'?root.onlyFans:{};state.version=Math.max(2,Number(state.version||1));if(!state.subscriptions||typeof state.subscriptions!=='object'||Array.isArray(state.subscriptions))state.subscriptions={};for(const [key,row] of Object.entries(state.subscriptions)){if(!row||typeof row!=='object'){delete state.subscriptions[key];continue}row.viewerIdentityId=String(row.viewerIdentityId||'');row.creatorIdentityId=String(row.creatorIdentityId||'');row.active=row.active!==false;row.startedAt=Math.max(0,Number(row.startedAt||Date.now()));row.updatedAt=Math.max(0,Number(row.updatedAt||Date.now()))}state.refresh=state.refresh&&typeof state.refresh==='object'?state.refresh:{lastAt:null,eventKeys:[],summary:''};state.refresh.eventKeys=Array.isArray(state.refresh.eventKeys)?state.refresh.eventKeys.slice(-180):[];state.refresh.summary=String(state.refresh.summary||'');root.onlyFans=state;return state;
}
...
function onlyFansAccountStats(identity,account=onlyFansAccount(identity,true)){
  const state=sharedOnlyFans(),named=Object.values(state.subscriptions).filter(x=>x.active&&x.creatorIdentityId===identity.id).length,manualPosts=(account.posts||[]).filter(x=>!x.seeded).length,subscribers=account.subscribersBase+named,monthly=Math.min(1000,account.monthlyEarningsBase+Math.round(named*account.subscriptionPrice*.32)),total=Math.min(10000,account.totalEarningsBase+Math.round(named*account.subscriptionPrice*.8));return{subscribers,posts:account.historicalPostCount+manualPosts,monthly,total};
}
...
function publishOnlyFansPostFor(creatorRef,data={},notify=true){
  const creator=resolveIdentityRef(creatorRef,false),account=onlyFansAccount(creator,false);if(!creator||!account)return null;const row=normalizeOnlyFansPost({...data,id:`onlyfans-post:${id()}`,identityId:creator.id,author:creator.name,timeMs:Number(data.timeMs||now().getTime()),seeded:false},creator);if(!row.visual&&!row.caption)return null;account.posts.unshift(row);account.posts=account.posts.slice(0,80);account.activity.unshift({id:id(),text:`Published a new ${row.type}.`,timeMs:row.timeMs});settingsRoot().onlyFansAccounts[creator.id]=account;ctx()?.saveSettingsDebounced?.();if(notify){const state=sharedOnlyFans();for(const sub of Object.values(state.subscriptions)){if(!sub.active||sub.creatorIdentityId!==creator.id)continue;const viewer=identityById(sub.viewerIdentityId);if(viewer)serviceNotification(viewer,'onlyfans',`${creator.name} posted`,row.caption||`New ${row.type}`,row.id)}}recordContinuityEvent({kind:'social',participants:[creator.name],sender:creator.name,summary:`${creator.name} published a new OnlyFans ${row.type}: ${row.visual}${row.caption?` | Caption: ${row.caption}`:''}`,threadTitle:'OnlyFans',mirrorId:row.id,roleplayMs:row.timeMs});render();return row;
}
function publishOnlyFansPost(data={}){return publishOnlyFansPostFor(currentIdentity(),data,true)}
function onlyFansPostSignature(creatorId,row={}){return hashString(JSON.stringify([creatorId,row.type==='video'?'video':'photo',lc(row.visual||row.mediaDescription),lc(row.caption)]))}
function normalizeOnlyFansRefreshResult(raw){let value=raw;if(typeof raw==='string'){try{value=parseJSON(raw)}catch{value=null}}if(!value||typeof value!=='object')return{summary:'',posts:[]};return{summary:String(value.summary||''),posts:Array.isArray(value.posts)?value.posts.filter(Boolean).slice(0,8):[]}}
function applyOnlyFansRefresh(result){const state=sharedOnlyFans(),keys=new Set(state.refresh.eventKeys||[]),creators=onlyFansCreators(),allowedById=new Map(creators.map(x=>[x.identity.id,x.identity])),allowedByName=new Map(creators.map(x=>[lc(x.identity.name),x.identity]));let added=0;for(const data of result.posts){const creator=allowedById.get(String(data.creatorIdentityId||''))||allowedByName.get(lc(data.creator));if(!creator)continue;const signature=onlyFansPostSignature(creator.id,data),account=onlyFansAccount(creator,false);if(keys.has(signature)||(account?.posts||[]).some(row=>onlyFansPostSignature(creator.id,row)===signature))continue;keys.add(signature);if(publishOnlyFansPostFor(creator,{type:data.type,visual:data.visual||data.mediaDescription,caption:data.caption,likes:Math.max(0,Number(data.likes||0)),comments:Math.max(0,Number(data.comments||0)),timeMs:now().getTime()},true))added++}state.refresh.lastAt=Date.now();state.refresh.summary=String(result.summary||'');state.refresh.eventKeys=[...keys].slice(-180);saveSharedOnlyFans(state);return added}
async function refreshOnlyFans(){
  if(onlyFansRefreshBusy||refreshBusy)return;if(!profile().apps.onlyfans){globalThis.toastr?.warning?.('OnlyFans is not installed on this phone.');return}const creators=onlyFansCreators();if(!creators.length){globalThis.toastr?.warning?.('Assign at least one creator first.');return}onlyFansRefreshBusy=true;islandText='Refreshing OnlyFans…';islandIcon=APPS.onlyfans.icon;render();
  try{const creatorContext=creators.map(({identity,account})=>({identityId:identity.id,name:identity.name,character:compactCard({name:identity.name,characterId:findCharacter(identity.name)?.index??null}),creatorProfile:{subscriptionPrice:account.subscriptionPrice,economicTier:account.economicTier,recentPosts:account.posts.slice(0,8).map(x=>({type:x.type,visual:x.visual,caption:x.caption}))},life:scopedWorld(world(),[identity.name])})),systemPrompt=`You generate one manual content refresh for a fictional adults-only creator platform inside an ongoing Greyhaven roleplay. Return only valid JSON, no markdown.
Every listed creator is confirmed age 18 or older. Use only ASSIGNED CREATORS and preserve each exact first/full name as supplied; never invent or expand a surname.
Create 1 to 4 fresh posts total, distributed naturally rather than forcing every creator to post. Match the creator card, confidence, public image, mood, relationships, current situation, and established content style. Avoid repeating recent posts.
Posts may range from ordinary lifestyle/behind-the-scenes material to sexually explicit consensual adult content when that genuinely fits the creator and their brand. Do not make every post the same lingerie mirror selfie. Do not include minors, coercion, incest, or non-consensual material.
The visual field must concretely describe what subscribers see so downstream AI understands the media. Captions should sound like the creator rather than generic promotional copy.
Required JSON: {"summary":"","posts":[{"creatorIdentityId":"exact assigned id","creator":"exact assigned name","type":"photo|video","visual":"concrete media description","caption":"in-character caption","likes":0,"comments":0}]}`,
    prompt=`ROLEPLAY TIME:\n${now().toISOString()}\n\nASSIGNED CREATORS:\n${JSON.stringify(creatorContext)}\n\nGenerate a fresh bounded batch now.`,raw=await generate({prompt,systemPrompt,responseLength:Math.max(1400,profile().responseTokens)}),added=applyOnlyFansRefresh(normalizeOnlyFansRefreshResult(raw));globalThis.toastr?.success?.(added?`OnlyFans refreshed · ${added} new post${added===1?'':'s'}`:'OnlyFans refreshed · no new posts')}
  catch(e){console.error(`[${GHP_MODULE}] OnlyFans refresh`,e);globalThis.toastr?.error?.(`OnlyFans refresh failed: ${e?.message||e}`)}finally{onlyFansRefreshBusy=false;islandText='';islandIcon='';render()}
}
...
function addDarkWebListing(state,data={}){
  const section=darkWebSection(data.section),role=darkWebRoleForSection(section),roles=globalAppRoles(),requestedExisting=data.providerType==='existing',existing=requestedExisting?(identityById(String(data.providerIdentityId||''))||identityForName(data.provider,{create:false})):null,allowed=existing&&roles.darkweb[role]?.[existing.id],provider=allowed?existing:createProvisionalIdentity(data.provider||data.firstName||'Vendor',{source:`darkweb-${section}`,adult:section==='escorts'}),title=String(data.title||'').trim(),price=normalizeMarketplacePrice(data.price);if(!provider||!title||!price)return null;const row=normalizeDarkWebListing({id:`darkweb:${id()}`,section,providerIdentityId:provider.id,providerName:provider.name,providerType:provider.kind==='provisional'?'provisional':'existing',title,description:String(data.description||'').trim(),price,duration:String(data.duration||'').trim(),area:String(data.area||'').trim(),visual:String(data.visual||'').trim(),mediaKey:String(data.mediaKey||''),mediaWidth:Number(data.mediaWidth||0),mediaHeight:Number(data.mediaHeight||0),ownerListing:data.ownerListing===true&&provider.kind!=='provisional',inquiries:[],timeMs:now().getTime(),status:'available'});if(state.listings.some(x=>x.signature===row.signature))return null;return row;
}
function normalizeDarkWebRefreshResult(raw){let value=raw;if(typeof raw==='string'){try{value=parseJSON(raw)}catch{value=null}}if(!value||typeof value!=='object')return{summary:'',listings:[],clientMessages:[]};return{summary:String(value.summary||''),listings:Array.isArray(value.listings)?value.listings.filter(Boolean).slice(0,12):[],clientMessages:Array.isArray(value.clientMessages)?value.clientMessages.filter(Boolean).slice(0,5):[]}}
function applyDarkWebRefresh(result){
  const state=sharedDarkWeb(),keys=new Set(state.refresh.eventKeys||[]),fresh=[];for(const data of result.listings){const row=addDarkWebListing(state,data);if(!row||keys.has(row.signature))continue;keys.add(row.signature);fresh.push(row)}if(fresh.length)state.listings.unshift(...fresh);let inquiries=0;
  for(const data of result.clientMessages){const listing=state.listings.find(x=>x.id===String(data.listingId||'')&&x.status==='available'&&x.ownerListing),seller=listing?identityById(listing.providerIdentityId):null,text=String(data.text||'').trim();if(!listing||!seller||!text)continue;const eventKey=hashString(JSON.stringify(['client',listing.id,lc(data.buyer),lc(text)]));if(keys.has(eventKey))continue;let buyer=data.buyerType==='existing'?(identityById(String(data.buyerIdentityId||''))||identityForName(data.buyer,{create:false})):null;if(!buyer||buyer.id===seller.id)buyer=createProvisionalIdentity(data.buyer||'Client',{source:'darkweb-client'});if(!buyer||buyer.id===seller.id)continue;const delivered=mirrorAppMessage({appName:'darkweb',fromName:buyer.name,toName:seller.name,fromIdentityId:buyer.id,toIdentityId:seller.id,type:'text',text,timeMs:now().getTime(),unread:true});if(!delivered?.delivered)continue;const sellerBox=phoneForIdentity(seller,true),thread=ensureAppThreadIn(sellerBox.timeline,'darkweb',buyer);thread.contextListingId=listing.id;sellerBox.root.phones[sellerBox.key]=sellerBox.timeline;saveMetadataRoot(sellerBox.root);listing.inquiries.push({id:eventKey,buyerIdentityId:buyer.id,buyerName:buyer.name,text,timeMs:now().getTime()});keys.add(eventKey);inquiries++
  }
  state.refresh.lastAt=Date.now();state.refresh.summary=String(result.summary||'');state.refresh.eventKeys=[...keys].slice(-180);saveSharedDarkWeb(state);return{listings:fresh.length,inquiries};
}
async function refreshDarkWeb(){
  if(darkWebRefreshBusy||refreshBusy)return;if(!profile().apps.darkweb){globalThis.toastr?.warning?.('Dark Web is not installed on this phone.');return}darkWebRefreshBusy=true;islandText='Refreshing Dark Web…';islandIcon=APPS.darkweb.icon;render();
  try{
    const providers=[];for(const section of ['drugs','escorts','crime','intel'])for(const {identity,config} of darkWebAssigned(section))providers.push({section,identityId:identity.id,name:identity.name,character:compactCard({name:identity.name,characterId:findCharacter(identity.name)?.index??null}),profile:config});const state=sharedDarkWeb(),existing=state.listings.map(x=>({id:x.id,section:x.section,provider:x.providerName,title:x.title,price:x.price,duration:x.duration,ownerListing:x.ownerListing})),ownerListings=state.listings.filter(x=>x.ownerListing&&x.providerType==='existing').map(x=>({id:x.id,section:x.section,ownerIdentityId:x.providerIdentityId,owner:x.providerName,title:x.title,description:x.description,price:x.price,duration:x.duration,area:x.area,existingInquiries:x.inquiries.slice(-5)})),buyerCandidates=identityDirectory().slice(0,30).map(x=>({identityId:x.id,name:x.name}));
    const systemPrompt=`You generate one manual refresh for a FICTIONAL Dark Web app inside Greyhaven roleplay. Return only valid JSON. No markdown.
...
Existing named provider characters may appear ONLY when explicitly included in ASSIGNED PROVIDERS for that same section. Never assign another real character. Provisional NPCs use ONE first name only; never invent or expand a surname.
Every escort is age 18 or older and every encounter is consensual. Include a fixed price, a duration such as 30 minutes, 1 hour, or one night, and clear fictional boundaries without coercion.
...
OWNER LISTINGS were manually published by assigned characters. Optionally create 0 to 3 plausible clientMessages showing genuine interest in one of them. Use an exact BUYER CANDIDATE identity or a provisional account with one first name. The message should reference the listing naturally and may ask a question, negotiate, or request a fictional meeting. Do not message removed listings.
Use fixed whole-euro prices, never ranges. Required JSON: {"summary":"","listings":[{"section":"drugs|escorts|crime|intel","providerType":"existing|provisional","provider":"Exact assigned name or one first name","providerIdentityId":"optional assigned id","title":"","description":"","price":"€0","duration":"","area":"","visual":"listing image description"}],"clientMessages":[{"listingId":"exact owner listing id","buyerType":"existing|provisional","buyer":"exact candidate name or one first name","buyerIdentityId":"optional exact candidate id","text":"natural private inquiry"}]}`;
    const prompt=`ROLEPLAY TIME:\n${now().toISOString()}\n\nASSIGNED PROVIDERS — exact allowlist:\n${JSON.stringify(providers)}\n\nOWNER LISTINGS AVAILABLE FOR INQUIRIES:\n${JSON.stringify(ownerListings)}\n\nBUYER CANDIDATES:\n${JSON.stringify(buyerCandidates)}\n\nGREYHAVEN LIFE:\n${JSON.stringify(scopedWorld(world(),providers.map(x=>x.name)))}\n\nEXISTING LISTINGS TO AVOID:\n${JSON.stringify(existing)}\n\nGenerate a balanced new batch now.`,raw=await generate({prompt,systemPrompt,responseLength:Math.max(1900,profile().responseTokens)}),result=applyDarkWebRefresh(normalizeDarkWebRefreshResult(raw)),parts=[];if(result.listings)parts.push(`${result.listings} new listing${result.listings===1?'':'s'}`);if(result.inquiries)parts.push(`${result.inquiries} client message${result.inquiries===1?'':'s'}`);globalThis.toastr?.success?.(`Dark Web refreshed · ${parts.join(' · ')||'nothing new'}`)
  }catch(e){console.error(`[${GHP_MODULE}] dark web refresh`,e);globalThis.toastr?.error?.(`Dark Web refresh failed: ${e?.message||e}`)}finally{darkWebRefreshBusy=false;islandText='';islandIcon='';render()}
}
...
function renderAppThread(appName){
  ...
  const bubbles=th.messages.slice(-100).map(m=>{
    ...
    const request=m.requestMedia?`<small class="ghp-msg-request"><i class="${m.requestMedia==='video'?'fa-solid fa-video':'fa-regular fa-image'}"></i> Requested a ${esc(m.requestMedia)}</small>`:'';
    return`<div class="ghp-msg ${mine?'mine':''} ${media?'has-media':''}" data-message-app="${esc(appName)}" data-thread-id="${esc(th.id)}" data-message-id="${esc(m.id)}" data-message-sender="${esc(m.sender)}">${content}${request}<time>${edited}${esc(timeText(new Date(m.timeMs)))}</time></div>`;
  }).join('');
  ...
  const armed=composeRequest.appName===appName&&composeRequest.threadId===th.id&&composeRequest.kind,requestBanner=armed?`<div class="ghp-request-banner"><i class="${armed==='video'?'fa-solid fa-video':'fa-regular fa-image'}"></i><span>Requesting a ${esc(armed)} · send your request next</span><button data-cancel-media-request><i class="fa-solid fa-xmark"></i></button></div>`:'';
  return`<div class="ghp-app ghp-thread ghp-social-thread ghp-${esc(appName)}-thread">${header(peer.name,blocked?'Blocked in this app':subtitle,`<span class="ghp-thread-tools">${video}<button data-social-thread-menu="${esc(appName)}" data-social-thread-id="${esc(th.id)}"><i class="fa-solid fa-ellipsis"></i></button></span>`)}<main data-conversation-scroll>${bubbles}${appReplyBusy?'<div class="ghp-typing"><i></i><i></i><i></i></div>':''}</main>${requestBanner}<form data-app-thread-form="${esc(appName)}" data-app-thread-id="${esc(th.id)}"><button type="button" class="ghp-plus" data-app-media="${esc(appName)}" data-app-media-thread="${esc(th.id)}" ${blocked||appReplyBusy?'disabled':''}><i class="fa-solid fa-plus"></i></button><input placeholder="${blocked?'Unavailable':armed?`Ask for the ${armed}…`:'Message…'}" ${blocked||appReplyBusy?'disabled':''}><button ${blocked||appReplyBusy?'disabled':''}><i class="fa-solid fa-arrow-up"></i></button></form></div>`;
}
...
function renderOnlyFansVideo(){
  const t=timeline(),th=appThread('onlyfans',itemId,t);if(!th){appView='messages';return renderOnlyFans()}const peer=appPeer(th)||{name:th.peerName,avatar:th.peerAvatar};return`<div class="ghp-of-video"><header><button data-end-onlyfans-video><i class="fa-solid fa-chevron-left"></i></button><span>${avatarHtml(peer,'small call-avatar')}<b>${esc(peer.name)}</b><small>Private video call</small></span><i class="fa-solid fa-video"></i></header><div class="ghp-of-video-stage">${avatarHtml(peer,'huge call-avatar')}<span>Live with ${esc(peer.name)}</span></div><main data-conversation-scroll>${th.videoTranscript.slice(-40).map(m=>`<div class="${lc(m.sender)===lc(persona().name)?'mine':''}"><b>${esc(m.sender)}</b><p>${esc(m.text||m.mediaDescription||'')}</p></div>`).join('')}${appReplyBusy?'<div class="ghp-typing"><i></i><i></i><i></i></div>':''}</main><form data-onlyfans-video-form="${esc(th.id)}"><input placeholder="Say or do something…" ${appReplyBusy?'disabled':''}><button ${appReplyBusy?'disabled':''}><i class="fa-solid fa-arrow-up"></i></button></form><button class="ghp-of-hangup" data-end-onlyfans-video><i class="fa-solid fa-phone-slash"></i></button></div>`;
}
function renderOnlyFans(){
  ...
  else{const age=identityAge(owner),eligible=age>=18;body=`<section class="ghp-role-settings"><i class="fa-solid fa-user-shield"></i><h2>Creator account</h2><p>This manual assignment follows ${esc(owner.name)} globally across chats. New creators receive plausible established history instead of starting at zero.</p><label><span><b>Subscription price</b><small>Fictional monthly payment</small></span><input class="onlyfans-price" type="number" min="1" max="500" step="0.01" value="${esc(ownAccount?.subscriptionPrice||12.99)}"></label><button class="ghp-wide-action ${ownAccount?'danger':''}" data-toggle-onlyfans-creator ${!eligible&&!ownAccount?'disabled':''}>${ownAccount?'Disable creator account':'Enable creator account'}</button><small>${eligible?`18+ eligibility confirmed from card (Age ${age}).`:'An explicit Age of 18 or older is required in the character card.'}</small></section>`}
  const right=`<span class="ghp-header-actions"><button data-onlyfans-refresh title="Generate fresh creator posts" ${onlyFansRefreshBusy?'disabled':''}><i class="fa-solid fa-rotate"></i></button><button data-app-view="settings"><i class="fa-solid fa-gear"></i></button></span>`;return`<div class="ghp-app ghp-social-app ghp-onlyfans">${header('OnlyFans','',right)}<main>${body}</main>${view==='settings'?'':socialTabs('onlyfans',tabs,view)}</div>`;
}
...
function renderDarkWeb(){
  const t=timeline(),view=appView||'drugs',owner=currentIdentity(),market=sharedDarkWeb(),roles=globalAppRoles(),sections={drugs:['Drug Market','fa-solid fa-capsules'],escorts:['Escorts','fa-solid fa-heart'],crime:['Crime for Hire','fa-solid fa-user-ninja'],intel:['Intel','fa-solid fa-eye']};if(view==='thread')return renderAppThread('darkweb');if(view==='listing'){const row=market.listings.find(x=>x.id===itemId);if(!row){appView='drugs';itemId='';return renderDarkWeb()}const provider=identityById(row.providerIdentityId)||{id:row.providerIdentityId,name:row.providerName},mine=row.ownerListing&&row.providerIdentityId===owner.id;return`<div class="ghp-app ghp-darkweb">${header(row.title,sections[row.section]?.[0]||'Dark Web')}<main class="ghp-market-detail">${socialMediaCard(row,row.title)}<h2>${esc(row.title)}</h2><h3>${esc(row.price)}${row.duration?` · ${esc(row.duration)}`:''}</h3><p>${esc(row.description)}</p><small>${esc(row.area||'Greyhaven')}</small><div class="ghp-market-seller">${avatarHtml(provider)}<span><b>${esc(provider.name)}</b><small>${row.providerType==='existing'?'Assigned Greyhaven provider':'Provisional identity · never auto-linked'}</small></span></div>${mine?`<button class="ghp-wide-action danger" data-remove-darkweb-listing="${esc(row.id)}"><i class="fa-solid fa-trash"></i> Remove listing</button>${row.inquiries.length?`<small>${row.inquiries.length} interested message${row.inquiries.length===1?'':'s'} received</small>`:''}`:`<button class="ghp-wide-action primary" data-darkweb-message="${esc(provider.id)}" data-darkweb-listing-context="${esc(row.id)}"><i class="fa-solid fa-comment-dots"></i> Message privately</button>`}</main></div>`}
  const tabs=[['drugs','fa-solid fa-capsules','Market'],['escorts','fa-solid fa-heart','Escorts'],['crime','fa-solid fa-user-ninja','Hire'],['intel','fa-solid fa-eye','Intel'],['messages','fa-solid fa-comments','Messages']];let body='';if(sections[view]){const listings=market.listings.filter(x=>x.section===view),assigned=view==='escorts'?darkWebAssigned('escorts'):[],canPublish=darkWebOwnedSections(owner).some(x=>x.section===view);body=`${view==='escorts'&&assigned.length?`<h3>Assigned profiles</h3><div class="ghp-dark-profiles">${assigned.map(({identity,config})=>`<button data-darkweb-provider="${esc(identity.id)}">${avatarHtml(identity,'large')}<span><b>${esc(identity.name)}</b><small>${esc(config.description||'Private companion profile')}</small><em>${esc(config.price||'Price on request')}${config.duration?` · ${esc(config.duration)}`:''}</em></span></button>`).join('')}</div>`:''}<div class="ghp-dark-heading"><span><b>${sections[view][0]}</b><small>${listings.length} active listings</small></span><span class="ghp-header-actions">${canPublish?`<button data-new-darkweb-listing="${esc(view)}" title="Publish as ${esc(owner.name)}"><i class="fa-solid fa-plus"></i></button>`:''}<button data-darkweb-refresh ${darkWebRefreshBusy?'disabled':''}><i class="fa-solid fa-rotate"></i></button></span></div><div class="ghp-market-grid ghp-dark-grid">${listings.map(darkWebListingCard).join('')||empty(sections[view][1],'No listings yet','Use the dedicated refresh or publish your own listing.')}</div>`}
  else if(view==='messages')body=appThreadRows('darkweb');
  else{const cfg=role=>roles.darkweb[role]?.[owner.id],dealer=cfg('dealers'),escort=cfg('escorts'),crime=cfg('crime'),intel=cfg('intel'),eligible=adultRoleEligible(owner);body=`<section class="ghp-role-settings ghp-dark-settings"><i class="fa-solid fa-user-secret"></i><h2>Manual provider roles</h2><p>Assignments are global. Refresh can use ${esc(owner.name)} only in explicitly enabled sections; it never assigns real characters automatically.</p><label><span><b>Drug-market dealer</b></span><input type="checkbox" data-dark-role="dealers" ${dealer?'checked':''}></label><label><span><b>Escort</b><small>${eligible?'18+ eligibility confirmed':'Requires explicit Age 18+'}</small></span><input type="checkbox" data-dark-role="escorts" ${escort?'checked':''} ${eligible?'':'disabled'}></label><label><span><b>Crime-for-hire provider</b></span><input type="checkbox" data-dark-role="crime" ${crime?'checked':''}></label><label><span><b>Intel broker</b></span><input type="checkbox" data-dark-role="intel" ${intel?'checked':''}></label><label class="stack"><span><b>Escort price</b></span><input class="dark-price" value="${esc(escort?.price||'€150')}"></label><label class="stack"><span><b>Duration</b></span><input class="dark-duration" value="${esc(escort?.duration||'1 hour')}"></label><label class="stack"><span><b>Public description / boundaries</b></span><textarea class="dark-description">${esc(escort?.description||'')}</textarea></label><button class="ghp-wide-action primary" data-save-dark-roles>Save global roles</button></section>`}
...
function render(){
  ...
  requestAnimationFrame(()=>{const scrollLatest=()=>{for(const s of o.querySelectorAll('[data-conversation-scroll]'))s.scrollTop=s.scrollHeight};scrollLatest();Promise.resolve(hydrateMediaPreviews()).finally(()=>requestAnimationFrame(scrollLatest))});
}
function goBack(){
...
function onlyFansPostPopup(){
  if(!onlyFansAccount(currentIdentity(),false))return;const d=popup(`<form method="dialog" class="ghp-media-form"><header><b>New OnlyFans post</b><button value="cancel"><i class="fa-solid fa-xmark"></i></button></header><label><span><b>Media type</b></span><select class="media-type"><option value="photo">Photo</option><option value="video">Video</option></select></label><label><span><b>What does it show?</b><small>This description is what the AI understands.</small></span><textarea class="media-description"></textarea></label><label><span><b>Caption</b></span><textarea class="media-caption"></textarea></label><label class="ghp-file-label"><span><b>Optional local media</b><small>Visual replacement only.</small></span><input class="media-file" type="file" accept="image/*,video/*"></label><button type="button" class="primary" data-publish-onlyfans>Post for subscribers</button></form>`);d.querySelector('[data-publish-onlyfans]').onclick=async()=>{const type=d.querySelector('.media-type').value,visual=d.querySelector('.media-description').value.trim(),caption=d.querySelector('.media-caption').value.trim(),file=d.querySelector('.media-file').files?.[0];if(!visual){globalThis.toastr?.warning?.('Describe the post media first.');return}const button=d.querySelector('[data-publish-onlyfans]');button.disabled=true;let mediaKey='',meta=null;if(file){mediaKey=await saveMediaBlob(file);meta=await fileMediaMeta(file,type)}if(publishOnlyFansPost({type,visual,caption,mediaKey,mediaWidth:meta?.width||0,mediaHeight:meta?.height||0})){d.close();app='onlyfans';appView='page';render()}};
}
function appendOnlyFansVideoTurn(fromRef,toRef,text){
  const from=resolveIdentityRef(fromRef,false),to=resolveIdentityRef(toRef,false),value=String(text||'').trim();if(!from||!to||!value)return null;const a=phoneForIdentity(from,true),b=phoneForIdentity(to,true);if(!a||!b)return null;const aThread=ensureAppThreadIn(a.timeline,'onlyfans',to),bThread=ensureAppThreadIn(b.timeline,'onlyfans',from),sessionId=String(aThread.videoSessionId||bThread.videoSessionId||`onlyfans-video-session:${id()}`),mirrorId=`onlyfans-video-turn:${id()}`,stamp=now().getTime(),entry=normalizeAppMessage({id:id(),mirrorId,sender:from.name,senderId:from.id,type:'text',text:value,timeMs:stamp,realMs:Date.now(),read:true,opened:true});
  for(const th of [aThread,bThread]){th.videoSessionId=sessionId;th.videoActive=true;if(!th.videoStartedAt)th.videoStartedAt=stamp;if(!th.videoTranscript.some(x=>x.mirrorId===mirrorId))th.videoTranscript.push({...clone(entry),id:id()})}
  noteRelationshipInteraction(a.timeline,to,'onlyfans',`${from.name} on video call: ${value}`,stamp);noteRelationshipInteraction(b.timeline,from,'onlyfans',`${from.name} on video call: ${value}`,stamp);a.root.phones[a.key]=a.timeline;a.root.phones[b.key]=b.timeline;saveMetadataRoot(a.root);recordContinuityEvent({kind:'call',participants:[from.name,to.name],sender:from.name,summary:`${from.name} said or did on a private OnlyFans video call: ${value}`,threadTitle:`OnlyFans video call with ${to.name}`,mirrorId,roleplayMs:stamp,transient:true});return entry;
}
function sendOnlyFansVideoTurn(threadKey,text){
  if(appReplyBusy)return false;const t=timeline(),th=appThread('onlyfans',threadKey,t),owner=currentIdentity(),peer=appPeer(th),value=String(text||'').trim();if(!th?.videoActive||!owner||!peer||!value)return false;if(!appendOnlyFansVideoTurn(owner,peer,value))return false;render();setTimeout(()=>generateAppReply('onlyfans',threadKey),180);return true;
}
function setOnlyFansVideoState(ownerRef,peerRef,active=true,startedAt=now().getTime()){
  ...
  const ownerThread=ensureAppThreadIn(ownerPhone.timeline,'onlyfans',peer),peerThread=ensureAppThreadIn(peerPhone.timeline,'onlyfans',owner),wasActive=ownerThread.videoActive||peerThread.videoActive,stamp=wasActive?Number(ownerThread.videoStartedAt||peerThread.videoStartedAt||startedAt):Number(startedAt||now().getTime()),sessionId=String(ownerThread.videoSessionId||peerThread.videoSessionId||`onlyfans-video-session:${id()}`);
  if(active&&!wasActive)for(const th of [ownerThread,peerThread]){th.videoTranscript=[];th.videoSessionId=sessionId}
  if(!active&&wasActive){const endedAt=now().getTime(),transcript=clone(ownerThread.videoTranscript.length?ownerThread.videoTranscript:peerThread.videoTranscript);for(const th of [ownerThread,peerThread]){th.videoCallHistory.unshift({id:sessionId,startedAt:stamp,endedAt,transcript:clone(transcript)});th.videoCallHistory=th.videoCallHistory.slice(0,12);th.videoTranscript=[]}}
  for(const th of [ownerThread,peerThread]){th.videoActive=!!active;th.videoStartedAt=active?stamp:0;th.videoSessionId=active?sessionId:''}
...
function darkWebOwnedSections(identity=currentIdentity()){
  if(!identity?.id)return[];const roles=globalAppRoles().darkweb,map=[['drugs','dealers','Drug Market'],['escorts','escorts','Escorts'],['crime','crime','Crime for Hire'],['intel','intel','Intel']];return map.filter(([,role])=>roles?.[role]?.[identity.id]).map(([section,role,label])=>({section,role,label,config:roles[role][identity.id]}));
}
function createDarkWebListingPopup(initialSection=''){
  const owner=currentIdentity(),sections=darkWebOwnedSections(owner);if(!sections.length){globalThis.toastr?.warning?.('Assign this character to at least one Dark Web section first.');return}const selected=sections.find(x=>x.section===initialSection)||sections[0],d=popup(`<form method="dialog" class="ghp-media-form"><header><b>Publish Dark Web listing</b><button value="cancel"><i class="fa-solid fa-xmark"></i></button></header><label><span><b>Section</b></span><select class="dark-listing-section">${sections.map(x=>`<option value="${esc(x.section)}" ${x.section===selected.section?'selected':''}>${esc(x.label)}</option>`).join('')}</select></label><label><span><b>Title</b></span><input class="dark-listing-title" placeholder="Private evening"></label><label><span><b>Description / boundaries</b></span><textarea class="dark-listing-description">${esc(selected.config?.description||'')}</textarea></label><label><span><b>Fixed price</b></span><input class="dark-listing-price" value="${esc(selected.config?.price||'€100')}" placeholder="€100"></label><label><span><b>Duration</b></span><input class="dark-listing-duration" value="${esc(selected.config?.duration||'')}" placeholder="1 hour"></label><label><span><b>Area</b></span><input class="dark-listing-area" value="Greyhaven"></label><label><span><b>Listing image description</b></span><textarea class="dark-listing-visual"></textarea></label><label class="ghp-file-label"><span><b>Optional local image</b><small>Visual replacement only.</small></span><input class="dark-listing-file" type="file" accept="image/*"></label><button type="button" class="primary" data-publish-darkweb-listing>Publish listing</button></form>`),select=d.querySelector('.dark-listing-section');select.addEventListener('change',()=>{const row=sections.find(x=>x.section===select.value);if(!row)return;d.querySelector('.dark-listing-description').value=row.config?.description||'';d.querySelector('.dark-listing-price').value=row.config?.price||'€100';d.querySelector('.dark-listing-duration').value=row.config?.duration||''});d.querySelector('[data-publish-darkweb-listing]').onclick=async()=>{const section=select.value,title=d.querySelector('.dark-listing-title').value.trim(),description=d.querySelector('.dark-listing-description').value.trim(),price=d.querySelector('.dark-listing-price').value.trim(),duration=d.querySelector('.dark-listing-duration').value.trim(),area=d.querySelector('.dark-listing-area').value.trim(),visual=d.querySelector('.dark-listing-visual').value.trim(),file=d.querySelector('.dark-listing-file').files?.[0];if(!title||!price){globalThis.toastr?.warning?.('Add a title and fixed price.');return}const button=d.querySelector('[data-publish-darkweb-listing]');button.disabled=true;let mediaKey='',meta=null;if(file){mediaKey=await saveMediaBlob(file);meta=await fileMediaMeta(file,'photo')}const state=sharedDarkWeb(),row=addDarkWebListing(state,{section,providerType:'existing',providerIdentityId:owner.id,provider:owner.name,title,description,price,duration,area,visual,mediaKey,mediaWidth:meta?.width||0,mediaHeight:meta?.height||0,ownerListing:true});if(!row){button.disabled=false;globalThis.toastr?.warning?.('That listing already exists or the role is not assigned.');return}state.listings.unshift(row);saveSharedDarkWeb(state);d.close();app='darkweb';appView=section;itemId='';render();globalThis.toastr?.success?.('Listing published across the shared Dark Web market.')};
}
function closeOwnedDarkWebListing(listingId){
  const state=sharedDarkWeb(),owner=currentIdentity(),index=state.listings.findIndex(x=>x.id===listingId&&x.providerIdentityId===owner?.id&&x.ownerListing);if(index<0)return false;const [row]=state.listings.splice(index,1);state.archived.unshift({...row,status:'removed',removedAt:Date.now()});state.archived=state.archived.slice(0,120);saveSharedDarkWeb(state);itemId='';appView=row.section;render();globalThis.toastr?.success?.('Dark Web listing removed.');return true;
}
...
function click(e){
  ...
  if(x.matches('[data-onlyfans-refresh]'))return refreshOnlyFans();
  ...
  if(x.dataset.darkwebMessage)return openAppConversation('darkweb',x.dataset.darkwebMessage,{listingId:x.dataset.darkwebListingContext||''});if(x.dataset.darkwebProvider)return openAppConversation('darkweb',x.dataset.darkwebProvider);if(x.dataset.newDarkwebListing)return createDarkWebListingPopup(x.dataset.newDarkwebListing);if(x.dataset.removeDarkwebListing){if(confirm('Remove this listing from the Dark Web market?'))closeOwnedDarkWebListing(x.dataset.removeDarkwebListing);return}if(x.matches('[data-save-dark-roles]'))return saveDarkWebRolesFromUi();
...
function submit(e){
  ...
  if(f.dataset.threadForm){const input=f.querySelector('input'),v=input.value;input.value='';const request=composeRequest.threadId===f.dataset.threadForm&&!composeRequest.appName?composeRequest.kind:'';composeRequest={threadId:'',kind:'',appName:''};sendThread(f.dataset.threadForm,v,'text',{requestMedia:request})}
  if(f.dataset.callForm){const input=f.querySelector('input'),v=input.value;input.value='';sendThread(f.dataset.callForm,v,'call')}
  if(f.dataset.appThreadForm){const input=f.querySelector('input'),value=input.value;input.value='';const request=composeRequest.appName===f.dataset.appThreadForm&&composeRequest.threadId===f.dataset.appThreadId?composeRequest.kind:'';composeRequest={threadId:'',kind:'',appName:''};sendAppMessage(f.dataset.appThreadForm,f.dataset.appThreadId,{type:'text',text:value,requestMedia:request})}
  if(f.dataset.onlyfansVideoForm){const input=f.querySelector('input'),value=input.value;input.value='';sendOnlyFansVideoTurn(f.dataset.onlyfansVideoForm,value)}
}
...
function expose(){
  globalThis.GreyhavenPhone={
    version:GHP_VERSION,open:openPhone,close:closePhone,refresh:refreshPhone,refreshMarketplace,refreshOnlyFans,refreshDarkWeb,
    ...
    apps:{
      ...
      createDarkWebListing:data=>{const state=sharedDarkWeb(),row=addDarkWebListing(state,data);if(!row)return null;state.listings.unshift(row);saveSharedDarkWeb(state);return clone(row)},closeDarkWebListing:closeOwnedDarkWebListing,
    },
  };
}
