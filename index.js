...
async function generateAppReply(appName,threadKey){
  ...
  try{
    const found=findCharacter(peer.name),w=world(),life=w.people.find(p=>lc(p.name)===lc(peer.name))||null,reln=t.relationships?.[peer.id]||null,listing=sharedMarketplace().listings.find(x=>x.sellerIdentityId===peer.id)||null;
...
function findAppThreadMessage(appName,tid,mid,t=timeline(false)){if(!['instagram','snapchat','facebook'].includes(appName))return null;const th=appStore(t,appName)?.threads?.[tid],m=th?.messages?.find(x=>x.id===mid);return th&&m?{th,m}:null}
